<template>
	<div class="recommend">
		...
		<div class="loda-container" v-if="!recommends.length">
			  ...
			  :key="index">
			  	<router-link
			  	  ...
			  	  :to="{name:'home-product', params:{id:item.baseinfo.itemId}}">
			  		...
			  		<p class="recommend-origPrice">￥{{item.price.origPrice}}</p>
		...
		methods:{
			...
			getRecommend(){
				...
				return getHomeRecommend(this.curPage).then(data => {
					// if(data){   //判断一下有没有正常获取到数据
					// 	this.curPage++; //当前页数逐渐增加
					// 	this.totalPage = data.totalPage; //更新当前总页数
					// 	this.recommends = this.recommends.concat(data.itemList); //一个一个合并获取
					// 	console.log(data);
